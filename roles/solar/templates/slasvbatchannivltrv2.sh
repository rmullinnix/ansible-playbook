...
#######################################################################
#  slasvbatchannivltr.sh           
#      This script is used to invoke Batch Anniversary Letter
#      
...
#  . .fbcspg03.profile
  . {{ solar_app_path }}/soloar/scripts/env_setup.sh
# 

export FILEPATH={{ solar_app_path }}/log/slasvbatchannivltr/
export PROCESSID=RNWLANIV1
export OUTFILE=slasvbatchannivltr.out
export ERRFILE=slasvbatchannivltr.err 
export LOGFILE=slasvbatchannivltr.log
...
# PACE pager stuff
########################################################################
export TMPFILE=slasvbatchannivltrtmp.log
export PACE=telalert@mom.assurant-inc.com
export PRIMARY="SolarFTPagerPri"
echo "Check slasvbatchannivltr.log--" $ENVIRON > $FILEPATH$TMPFILE
...
JOBSTRING="Batch Anniversary Letter"
if [ $Ret -eq 0 ] 
then
