...
#      This script is used to invoke the Batch Renewal Export to COMPASS
#      
#      Runs daily - Monday thru Friday-Autosys job name P3SLDSCXL
#  
#   Revision History:
#   06/02/2014 L. Harnack         Initial Development (Linux version)
########################################################################
...
#  if you run this script under your own id, you'll need to comment out these lines
#  . .fbcspg03.profile
  . {{ solar_app_path }}/soloar/scripts/env_setup.sh $AS_ENV $AS_APP
# 

export FILEPATH={{ solar_app_path }}/log/slasvbatchrnwlexportsc/
export PROCESSID=RNWLSCX
export OUTFILE=slasvbatchrnwlexportsc.out
export ERRFILE=slasvbatchrnwlexportsc.err 
export LOGFILE=slasvbatchrnwlexportsc.log
...
# PACE pager stuff
########################################################################
export TMPFILE=slasvbatchrnwlexportsctmp.log
export PACE=telalert@mom.assurant-inc.com
export PRIMARY="SolarFTPagerPri"
echo "Check slasvbatchrnwlexportsc.log--" $ENVIRON > $FILEPATH$TMPFILE
...
JOBSTRING="Batch Renewal Export to Compass"
if [ $Ret -eq 0 ] 
then
...
#   Remove Log files over 60 days old (runs weekly - keep 2 months)
########################################################################
